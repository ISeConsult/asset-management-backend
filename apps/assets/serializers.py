...
class AssetManufacturerListSerializer(serializers.ModelSerializer):
    ...
    def get_accessories(self, obj):
        accessory = Asset.objects.filter(manufacturer=obj, category__name="accessories")
        if accessory:
...
class AssetLocationListSerializer(serializers.ModelSerializer):
    ...
    def get_consumables(self, obj):
        consumable = Asset.objects.filter(location=obj, category__name="consumables")
        if consumable:
            return consumable.count()
        return 0

    def get_accessories(self, obj):
        accessory = Asset.objects.filter(location=obj, category__name="accessories")
        if accessory:
            return accessory.count()
        return 0
...
class CompanyListSerializer(serializers.ModelSerializer):
    ...
    def get_consumables(self, obj):
        consumable = Asset.objects.filter(company=obj, category__name="consumables")
        if consumable:
            return consumable.count()
        return 0

    def get_accessories(self, obj):
        accessory = Asset.objects.filter(company=obj, category__name="accessories")
        if accessory:
            return accessory.count()
        return 0
...
class AssetSupplierListSerializer(serializers.ModelSerializer):
    ...
    def get_consumables(self, obj):
        consumable = Asset.objects.filter(supplier=obj, category__name="consumables")
        if consumable:
            return consumable.count()
        return 0

    def get_accessories(self, obj):
        accessory = Asset.objects.filter(supplier=obj, category__name="accessories")
        if accessory:
            return accessory.count()
        return 0
...
class ComponentCheckOutCreateUpdateSerializer(serializers.ModelSerializer):
    class Meta:
...
class ComponentCheckOutListSerializer(serializers.ModelSerializer):
    ...
    def get_component(self, obj):
        if obj.component:
            return ComponentsListSerializer(obj.component).data
        return None

    def get_component_request(self, obj):
        if obj.component_request:
            return ComponentRequestListSerializer(obj.component_request).data
        else:
            return None
