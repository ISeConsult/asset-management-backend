...
class LicenseViewset(viewsets.ModelViewSet):
    ...
    def create(self, request, *args, **kwargs):
        try:
            ...
            return Response(
                {"success": True, "info": "License Created Successfully"},
                status=status.HTTP_201_CREATED,
            )
...
class LicenseCheckoutViewset(viewsets.ModelViewSet):
    ...
    def create(self, request, *args, **kwargs):
        try:
            ...
            if not lisc:
                return Response(
                    {"success": False, "info": "license cannot be found"},
                    status=status.HTTP_400_BAD_REQUEST,
                )

            if lisc.licensed_to:
                return Response(
                    {"success": False, "info": "license has been checked out"},
                    status=status.HTTP_400_BAD_REQUEST,
                )
            ...
            if not user_data:
                Response(
                    {"success": False, "info": "User cannot be found"},
                    status=status.HTTP_400_BAD_REQUEST,
                )
            ...
            LicenseHistory.objects.create(
                license=lisc, user=user_data, action="checked_out"
            )

            return Response(
                {"success": True, "info": "License Checked out successfully"},
                status=status.HTTP_201_CREATED,
            )
...
class LicenseHistoryViewset(viewsets.ModelViewSet):
    ...
    def create(self, request, *args, **kwargs):
        try:
            ...
            return Response(
                {"success": True, "info": "License History Created successfully"},
                status=status.HTTP_201_CREATED,
            )
