...
class License(models.Model):
    ...
    company = models.ForeignKey(
        "assets.Company", on_delete=models.CASCADE, null=True, blank=True
    )
    manufacturer = models.ForeignKey(
        "assets.AssetManufacturer", on_delete=models.CASCADE
    )
    order_number = models.CharField(max_length=120)
    licensed_to_email = models.EmailField(null=True, blank=True)
    licensed_to = models.ForeignKey(
...
class LicenseCheckOut(models.Model):
    ...
    user = models.ForeignKey(
        "people.User", on_delete=models.CASCADE, related_name="license_checkout_user"
    )
    checkout_by = models.ForeignKey(
        "people.User",
        on_delete=models.CASCADE,
        related_name="license_checkout_checkout_by",
    )
    note = models.TextField(null=True, blank=True)
    created_at = models.DateTimeField(auto_now_add=True)
    updated_at = models.DateTimeField(auto_now=True)
...
class LicenseHistory(models.Model):
    ...
    license = models.ForeignKey(License, on_delete=models.CASCADE)
    user = models.ForeignKey(
        "people.User", on_delete=models.CASCADE, related_name="license_history_user"
    )
